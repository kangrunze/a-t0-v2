...
# 与 _load_at0_data 同样的兜底逻辑：Trae 沙箱加密 __init__.py 时直载 paths.py。
def _load_at0_paths():
    try:
...
DATA_ROOT, DEFAULT_DATA_DIR = _load_at0_paths()
...
# DEFAULT_DATA_DIR 来自 _load_at0_paths()（见文件顶部）
...
def main() -> int:
    ...
    parser.add_argument("--data-dir", default=str(DEFAULT_DATA_DIR),
                        help=f"本地数据目录，默认 {DEFAULT_DATA_DIR}")
