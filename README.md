...
> 通过 60 日振幅筛选（Youden 最优阈值 0.0494）过滤低波动股票，提升净盈亏与成本/毛利比。
...
| `min_amplitude_long` | 0.0494 | **60 日日均振幅下限（Youden 最优阈值）** |

**`min_amplitude_long` 验证依据**（按证据强度排序）：

1. **【样本外·主依据】F5 500 股全池验证**：27 只独立股票净盈亏 +128,622，盈利占比 70.4%，成本/毛利比 42.0%，4/4 标准全通过
2. **【样本外·辅证】Stage D 步骤2**：50 只独立股票，6/6 标准全通过
3. **【训练集·参考】100 股×3 年 A/B**：净盈亏 -82,218 → +22,959（有偏乐观风险，不作为主依据）
