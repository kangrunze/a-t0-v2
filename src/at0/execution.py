...
try:
    from .paths import (
        POSITIONS_FILE, POSITIONS_LOCK_FILE, LIVE_OPEN_LEGS_FILE,
    )
except (ImportError, ValueError):
    POSITIONS_FILE = PROJECT_ROOT / "data" / "positions.json"
    POSITIONS_LOCK_FILE = POSITIONS_FILE.with_suffix(".json.lock")
    LIVE_OPEN_LEGS_FILE = PROJECT_ROOT / "data" / "live_open_legs.json"
LOCK_FILE = POSITIONS_LOCK_FILE  # 向后兼容别名
...
# LIVE_OPEN_LEGS_FILE 已在文件顶部路径配置块从 at0.paths 导入（含 fallback）。
