...
# ═══════════════════════════════════════════════════════════════
# 路径配置（统一从 at0.paths 获取，支持 AT0_DATA_DIR 环境变量覆盖）
# ═══════════════════════════════════════════════════════════════
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent  # src/at0/ -> src/ -> 项目根
try:
    from at0.paths import (
        DATA_ROOT, MINUTE_BARS_DIR, MULTI_DAY_CACHE_DIR, THEMES_FILE,
    )
except (ImportError, ValueError):
    # importlib 直载本模块时（Trae 沙箱外），at0 包未初始化，回退直载 paths.py
    import importlib.util as _ilu
    _paths_path = Path(__file__).resolve().parent / "paths.py"
    _spec = _ilu.spec_from_file_location("_at0_paths_local", str(_paths_path))
    _paths_mod = _ilu.module_from_spec(_spec)
    _spec.loader.exec_module(_paths_mod)
    DATA_ROOT = _paths_mod.DATA_ROOT
    MINUTE_BARS_DIR = _paths_mod.MINUTE_BARS_DIR
    MULTI_DAY_CACHE_DIR = _paths_mod.MULTI_DAY_CACHE_DIR
    THEMES_FILE = _paths_mod.THEMES_FILE
MINUTE_DATA_DIR = MINUTE_BARS_DIR  # 向后兼容别名
